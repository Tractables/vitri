...
| `VITRI_GOATD_CANDIDATES` | how many of the goatd schedule's decompositions its portfolio entry converts and offers for selection: the winner, refined, then runners-up unrefined, drawn in turn from each kind of candidate the run produced; each runner-up is published as `candidate=n` of the entry's spec | `1` to `8` | `1` |
