...
| `VITRI_GOATD_CANDIDATES` | how many of the goatd schedule's decompositions its portfolio entry converts and offers for selection: the winner, refined, then its runners-up unrefined; each runner-up is published as `candidate=n` of the entry's spec | `1` to `8` | `1` |
