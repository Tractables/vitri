...
// Absolute CLOCK_MONOTONIC seconds. Deliberately the SAME quantity that
// CryptoMiniSat's real_time_sec() returns, so a deadline computed by the
// caller is directly comparable here without any epoch conversion.
...
// Stops the CaDiCaL search once a wall-clock deadline has passed.
//
// CaDiCaL polls its terminator from the search loop and from every
// inprocessing pass (Internal::terminated_asynchronously), and a terminated
// solve() returns 0 — exactly the return code CadiBack already interprets as
// "conflict limit hit", which sets `limit_hit` and makes every backbone loop
// break out with the backbone found SO FAR. So a deadline hit yields a
// PARTIAL backbone through a path the conflict limit already exercises; no
// new control flow and no change to what a completed run returns.
//
// The terminator is connected only when a deadline was actually requested, so
// a default (deadline-free) call is bit-identical to before this patch.
struct DeadlineTerminator : public CaDiCaL::Terminator {
  double deadline;
  DeadlineTerminator (double d) : deadline (d) {}
  bool terminate () override { return monotonic_seconds () > deadline; }
};
...
// Set to true if the conflict limit or the wall-clock deadline was hit
// during backbone search.
bool limit_hit = false;

// Wall-clock deadline as absolute CLOCK_MONOTONIC seconds;
// numeric_limits<double>::max() (the default) means "no deadline".
double deadline = std::numeric_limits<double>::max ();
DeadlineTerminator terminator{std::numeric_limits<double>::max ()};
...
int solve () {
  ...
  // Same shape as the conflict limit above, one step earlier: if the deadline
  // has already passed there is no point entering the solver at all. The
  // in-flight case is covered by the connected DeadlineTerminator.
