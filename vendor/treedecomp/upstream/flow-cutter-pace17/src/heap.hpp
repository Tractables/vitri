...
public:
	...
	// regression can unit-test the arithmetic without allocating a
	// multi-gigabyte heap.
	static long long children_begin(long long pos){
		return static_cast<long long>(k)*pos + 1;
	}

	static long long children_end(long long pos){
		return static_cast<long long>(k)*(pos+1) + 1;
	}
...
private:
	...
	void move_down(int pos){
		...
		for(;;){
			// 64-bit: children_begin/end can exceed INT_MAX for large pos (see
			// the helpers' note). heap_end fits in int, so after the min() clamp
			// begin/end are back in [0, heap_end] and safe to index — only the
			// pre-clamp child-index product must be computed in 64-bit.
			long long begin = std::min<long long>(heap_end, children_begin(pos));
			long long end = std::min<long long>(heap_end, children_end(pos));

			if(begin == end)
				break;

			long long min_child_pos = begin;
			...
			pos = static_cast<int>(min_child_pos);
		}
		heap[pos].id = id;
		heap[pos].key = std::move(key);
		id_pos[id] = pos;
