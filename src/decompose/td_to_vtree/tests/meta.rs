...
fn bag_rank_is_bottom_up_topological() {
    ...
    let meta = BagMetadata::from_assignment(3, &[0, 1, 2], &[0, 1, 2], 3, 2);
    ...
    assert_eq!(
        meta.max_bag_size(),
        2,
        "the largest bag comes from the decomposition, not from the one-bag-per-variable assignment",
    );
}

#[test]
fn unassigned_var_has_no_rank() {
    let meta = BagMetadata::from_assignment(2, &[usize::MAX, 0], &[0], 1, 1);
