...
fn survivors_are_renumbered_in_ascending_order_and_invert_back() {
    ...
    assert_eq!(r.kept(), &[v(2), v(4), v(5)]);

    assert_eq!(r.new_id(v(2)), Some(v(1)));
    assert_eq!(r.new_id(v(4)), Some(v(2)));
    assert_eq!(r.new_id(v(5)), Some(v(3)));
    for new in 1..=3 {
        assert_eq!(r.new_id(r.old_id(v(new))), Some(v(new)));
    }

    // Dropped variables, and ids the old formula never had, have no new name.
    assert_eq!(r.new_id(v(1)), None);
    assert_eq!(r.new_id(v(3)), None);
...
fn renumbering_a_literal_never_flips_its_polarity() {
    ...
    assert_eq!(r.apply_lit(Literal::pos(v(1))), None);

    assert_eq!(r.apply_inverse_lit(Literal::neg(v(2))), Literal::neg(v(3)));
    assert_eq!(r.apply_inverse_lit(Literal::pos(v(2))), Literal::pos(v(3)));
}

#[test]
fn a_clause_that_loses_every_literal_survives_as_the_empty_clause() {
    // Eliminating variable 1 empties the first clause, which is the UNSAT
...
fn composing_two_renumberings_names_each_survivor_by_its_original_variable() {
    ...
    assert_eq!(composed.kept(), &[v(4), v(6)]);
    assert_eq!(composed.new_id(v(4)), Some(v(1)));
    assert_eq!(composed.new_id(v(6)), Some(v(2)));
    assert_eq!(
        composed.new_id(v(2)),
        None,
